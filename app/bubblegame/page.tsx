...
export default function BubbleShooter() {
  ...
  useEffect(() => {
    ...
    // Draw floating bubbles background effect
    for (let i = 0; i < 50; i++) {
      ctx.beginPath();
    ...
    // Draw bubbles with realistic 3D gradient
    bubbles.forEach((b) => {
      ...
      ctx.beginPath();
      ctx.arc(b.x - bubbleRadius/3, b.y - bubbleRadius/3, bubbleRadius/5, 0, Math.PI*2);
    ...
    // Draw bullet
    if(bullet){
      const bGradient = ctx.createRadialGradient(bullet.x-5, bullet.y-5, 2, bullet.x, bullet.y, bubbleRadius);
      bGradient.addColorStop(0,"white");
      bGradient.addColorStop(0.2, bullet.color);
      ...
      // small sparkle
      ctx.beginPath();
      ctx.arc(bullet.x - bubbleRadius/3, bullet.y - bubbleRadius/3, bubbleRadius/5, 0, Math.PI*2);
      ctx.fillStyle = "rgba(255,255,255,0.7)";
      ctx.fill();
      ctx.closePath();
    }
  ...
  const findConnected = (start, allBubbles) => {
  ...
  // Snap bullet to nearest grid
  const snapToGrid = (x,y)=>{
    let minDist=Infinity, nearestX=x, nearestY=y;
    bubbles.forEach(b=>{
      const steps=[[0,-bubbleRadius*Math.sqrt(3)],[bubbleRadius*2,0],[-bubbleRadius*2,0],[bubbleRadius,-bubbleRadius*Math.sqrt(3)],[-bubbleRadius,-bubbleRadius*Math.sqrt(3)],[bubbleRadius,bubbleRadius*Math.sqrt(3)],[-bubbleRadius,bubbleRadius*Math.sqrt(3)]];
      steps.forEach(([dx,dy])=>{
        const nx = b.x + dx;
        const ny = b.y + dy;
      ...
      setBullet(prev=>{
        ...
        for(let b of bubbles){
          if(Math.hypot(newX-b.x,newY-b.y)<bubbleRadius*2){
            ...
            const newBubble = {x:snapped.x, y:snapped.y, color:prev.color};
            let newBubbles=[...bubbles,newBubble];
            const connected = findConnected(newBubble,newBubbles);
            if(connected.length>=3) newBubbles=newBubbles.filter(bb=>!connected.includes(bb));
  ...
  // Shoot
  const shootAt=(mouseX,mouseY)=>{
    if(bullet||!nextBubble||gameOver) return;
    const angle = Math.atan2(mouseY-shooter.y, mouseX-shooter.x);
    setShooter(s=>({...s, angle}));
    const speed = 8;
    setBullet({x:shooter.x, y:shooter.y, dx:Math.cos(angle)*speed, dy:Math.sin(angle)*speed, color:nextBubble.color});
    setNextBubble({color:colors[Math.floor(Math.random()*colors.length)]});
  };
    ...
    <div className="game-container">
      ...
      {!gameOver && nextBubble && (
        <div className="next-bubble-container">
          <p>Next Bubble:</p>
          <div style={{
            width:40, height:40, borderRadius:"50%", margin:"10px auto",
            background: nextBubble.color, boxShadow:"0 0 10px white"
          }}></div>
        </div>
      )}
